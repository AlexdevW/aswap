...
describe('Pool', function () {
  ...
  it('pool info', async function () {
    const { pool, token0, token1, tickLower, tickUpper, fee, sqrtPriceX96 } = await loadFixture(deployFixture)
  ...
  it('mint and burn and collect', async function () {
    const { pool, token0, token1, sqrtPriceX96 } = await loadFixture(deployFixture)
    ...
    // console.log(await token0.read.balanceOf([testLP.address]), initBalanceValue, 'initBalanceValue')
  ...
  it('swap', async function () {
    ...
    // mint 多一些流动性，确保交易可以完全完成
    await testLP.write.mint([testLP.address, liquidityDelta, pool.address, token0.address, token1.address])
    ...
    const costToken0 = 300n * 10n ** 18n - ((await token0.read.balanceOf([testSwap.address])) as bigint)
    ...
    // 查看当前 token 数量
    expect(await token0.read.balanceOf([testLP.address])).to.equal(99995000161384542080378486215n)
    // 提取 token
    await testLP.write.collect([testLP.address, pool.address])
    // 判断 token 是否返回给 testLP，并且大于原来的数量，因为收到了手续费，并且有交易换入了 token0
    // 初始的 token0 是 const initBalanceValue = 100000000000n * 10n ** 18n;
    expect(await token0.read.balanceOf([testLP.address])).to.equal(100000000099999999999999999998n)
  })
})
