...
describe("Pool", function () {
  async function deployFixture() {
    ...
    const poolAddress: `0x${string}` =
      (createEvents[0].args as any).pool || "0x";
  ...
  it("mint and burn and collect", async function () {
    ...
    console.log(await token0.read.balanceOf([pool.address]), 'await token0.read.balanceOf([pool.address])', await token1.read.balanceOf([pool.address]), sqrtPriceX96)
    expect(await token0.read.balanceOf([pool.address])).to.equal(
      initBalanceValue -
        ((await token0.read.balanceOf([testLP.address])) as bigint)
    );
    expect(await token1.read.balanceOf([pool.address])).to.equal(
      initBalanceValue -
        ((await token1.read.balanceOf([testLP.address])) as bigint)
    );
    ...
    const totalToken0 =
      ...
      ((await token0.read.balanceOf([testLP.address])) as bigint) +
      (initBalanceValue -
        ((await token0.read.balanceOf([testLP2.address])) as bigint));
