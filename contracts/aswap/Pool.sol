...
contract Pool is IPool {
    ...
    struct SwapState {
        ...
        // 这个变量用于跟踪用户在交易中还剩下多少可以使用的代币。
        int256 amountSpecifiedRemaining;
        // the amount already swapped out/in of the output/input asset
        // 这个变量用于跟踪用户在交易后实际上得到了多少代币，并考虑了手续费的影响。
    ...
    function swap(
        // 交易的接收地址，即谁将收到交易的结果
        address recipient,
        // 一个布尔值，指示交易的方向 (如果为 true，则表示从 token0 交换到 token1, 反之 相反)
        bool zeroForOne,
        // 一个整数值，表示用户希望交换的代币数量。如果这个值大于 0，表示用户想用 token0 进行交换；如果小于 0，表示用户想用 token1 进行交换
        int256 amountSpecified,
        // 一个整数值，表示允许的价格限制。这个价格限制用于防止交换价格超过预定范围。
        ...
        if (exactInput) {
            // 表示用户指定了输入代币数量（即用户想要用多少 token0 交换 token1）
            ...
            // 表示用户指定了输出代币数量（即用户想要得到多少 token1）
