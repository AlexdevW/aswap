...
describe('PositionManager', function () {
  ...
  it('mint && burn && collect', async function () {
    const { manager, sender, token0, token1, pool } = await loadFixture(deployFixture)
    ...
    // mint 成功，检查余额
    expect(await token0.read.balanceOf([sender])).to.equal(999949496579641839195n)
    expect(await token0.read.balanceOf([pool.address])).to.equal(50503420358160805n)
  ...
  it('collet with fee', async function () {
    const { pool, token0, token1, manager, sender } = await loadFixture(deployFixture)
    ...
    // 查看当前 token 数量
    expect(await token0.read.balanceOf([sender])).to.equal(99994949657964183919574228499n)

    // 提取 token
    await manager.write.collect([1n, sender])
    // 判断 token 是否返回给 testLP，并且大于原来的数量，因为收到了手续费，并且有交易换入了 token0
    // 初始的 token0 是 const initBalanceValue = 100000000000n * 10n ** 18n;
    expect(await token0.read.balanceOf([sender])).to.equal(100000000099949495579641839194n)

    await manager.write.collect([2n, sender])
    expect(await token0.read.balanceOf([sender])).to.equal(100000000099999999999999999997n)
  })
})
