...
describe('SwapRouter', function () {
  async function deployFixture() {
    ...
    const swapRouter = await viem.deployContract('SwapRouter', [poolManager.address])
    ...
    // 给池子1注入流动性
    // 获取池子1的地址
    const pool1Address = await poolManager.read.getPool([token0.address, token1.address, 0])
    // 给池子1注入流动性
    await token0.write.approve([pool1Address, initBalanceValue])
    await token1.write.approve([pool1Address, initBalanceValue])
    await testLP.write.mint([testLP.address, 50000n * 10n ** 18n, pool1Address, token0.address, token1.address])

    // 给池子2注入流动性
    // 获取池子2的地址
    const pool2Address = await poolManager.read.getPool([token0.address, token1.address, 1])
    // 给池子2注入流动性
    await token0.write.approve([pool2Address, initBalanceValue])
    await token1.write.approve([pool2Address, initBalanceValue])
    await testLP.write.mint([testLP.address, 50000n * 10n ** 18n, pool2Address, token0.address, token1.address])
  ...
  it('exactOutput', async function () {
    ...
    expect(1000000000000n * 10n ** 18n - token0Amount).to.equal(1003011033103311n)
    // 检查收到的 tokenOut 数量
    const token1Amount = await token1.read.balanceOf([sender])
