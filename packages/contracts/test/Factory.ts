...
describe('Factory', function () {
  ...
  it('createPool with same token', async function () {
    ...
    await expect(factory.write.createPool([tokenA, tokenB, 1, 100000, 3000])).to.be.rejectedWith('IDENTICAL_ADDRESSES')

    await expect(factory.read.getPool([tokenA, tokenB, 3])).to.be.rejectedWith('IDENTICAL_ADDRESSES')
  })
})
